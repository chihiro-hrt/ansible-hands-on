...
```yaml
  ...
  - name: save to localfile
    copy:
      content:  "{{ (result.stdout_lines[4] + result.stdout_lines[5] ) | join('\n') }}"
...
- content  
ファイル内容を指定します。  
`show port`、`show vlan`を指定する為、Playbook実行結果として格納される`stdout_lines[4]`、`stdout_line[5]`をそれぞれ指定します。
- dest  
...
```
$ cat ax_commands.txt 
Date 1980/05/06 04:58:53 UTC
Port Counts: 28
Port  Name           Status  Speed           Duplex     FCtl FrLen  ChGr/Status
 0/1  gigaether0/1   up      100BASE-TX      full(auto) off   1518  -/-
 0/2  gigaether0/2   down    -               -          -        -  -/-
 ～抜粋～
 0/28 gigaether0/28  down    -               -          -        -  -/-
Date 1980/05/06 04:58:53 UTC
VLAN counts: 1
...
```yaml
  ...
  - name: save running-config to localfile
    copy:
      ...
      dest: './config_{{ ansible_net_hostname }}_{{ now(False, "%Y%m%d_%H%M%S") }}.log'
```
■Playbook内容の説明
...
`now(False, "%Y%m%d_%H%M%S")`で時刻情報を指定しています。
...
$ ls -l
～抜粋～
-rw-rw-r-- 1 nakayama nakayama 2080  1月 15 18:06 config_AX2230_20200115_180642.txt
