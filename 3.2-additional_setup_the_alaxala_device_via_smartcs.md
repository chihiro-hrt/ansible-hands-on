...
PLAY RECAP ***********************************************************************************************
ax2230                     : ok=1    changed=0    unreachable=0    failed=0    skipped=0    rescued=0    ignored=0   
```
...
PLAY RECAP ************************************************************************************************
ax2230                     : ok=3    changed=4   unreachable=0    failed=0    skipped=0    rescued=0    ignored=0   
...
■Playbook(add-servers.yml)  
```
---
- name: add ntp, snmp, and syslog server
  hosts: ax
  gather_facts: no
...
■Playbook内容の説明  
- with_items:  
<code>group_vars/ax.yml</code>で定義した内容のうち、ループ処理する要素を指定します。  
...
■実行例  
```
$ ansible-playbook add_servers.yml
```

■実行結果例    
```
PLAY [add ntp, snmp, and syslog server] *******************************************************************

TASK [configure ntp server] *******************************************************************************
changed: [ax2230] => (item=10.5.28.115)
changed: [ax2230] => (item=10.5.31.24)

TASK [configure snmp community] ***************************************************************************
changed: [ax2230] => (item={'name': 'public', 'auth': 'ro'})
changed: [ax2230] => (item={'name': 'secret', 'auth': 'rw'})
...
PLAY RECAP ************************************************************************************************
ax2230                     : ok=3    changed=7    unreachable=0    failed=0    skipped=0    rescued=0    ignored=0   
...
STEP3のPlaybookは、<code>group_var/ax.yml</code>に設定する内容を記載していますが、１つのPlaybookにまとめる事もできます。  
...
ただできればSTEP3のように変数の定義を分けておく方が各パラメータに変更が発生した場合にも修正量が少なくて済みます。
