...
- name: '3-1 step2'
  ...
  - name: "smartcs_tty_command"
    smartcs_tty_command:
      ...
      - 'exit'
```
> registerしてdebugでstdout_lines_customを表示させる予定  
> custom_responseオプションの説明追記
<br>
...
ただし、<code>smartcs_tty_command</code>を使ったPlaybookを作成する際は、これまでSTEP1、STEP2で演習した通りコンソールの入出力情報を全て理解していないとPlaybook作成ができないという課題があります。  
Ansibleモジュールが無いネットワーク機器の場合は<code>smartcs_tty_command</code>を使った作成しか方法はありませんが、Ansibleモジュールがあるネットワーク機器の場合、<code>ax_config</code>をSmartCS経由で実行するというやり方が可能です。
