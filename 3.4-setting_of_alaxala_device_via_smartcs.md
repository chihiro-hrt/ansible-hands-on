...
PLAY RECAP ***********************************************************************************************
ns2250                     : ok=4    changed=0    unreachable=0    failed=0    skipped=0    rescued=0    ignored=0   
```
...
- <code>vars</code>に<code>ansible_user: "port1"</code>、<code>ansible_password: "port1"</code>、<code>ansible_port=9301</code>を追記
...
それぞれ一長一短があります。  
ターゲットとなるネットワーク機器のベンダーモジュールがあり、コネクションプラグインとして<code>network_cli</code>をサポートしてる場合は、Playbookの再利用性などを考え、(2)の方法で連携する方が望ましいです。
...
|△ |コンソールの入出力情報がないと、Playbook作成ができない。冪等性の担保ができない。 |<code>network_cli</code>をサポートしている必要がある。処理時間がSSH経由と比較して長い |

(1)<code>smartcs_tty_command</code>を使う場合  
- ベンダー製モジュールの無いネットワーク機器に対してもコンソール経由でアクセスすることが可能  
- コンソールの入出力情報が必要な為、Playbook作成の難易度が高い（面倒）  

(2)他ベンダーモジュールとSmartCSを連携させる場合  
- コネクションプラグインで<code>network_cli</code>をサポートしているベンダー製モジュールと連携が可能
- Playbookのtask部分について使いまわしが可能
